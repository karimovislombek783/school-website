...
export function newsletterConfigured() {
  return Boolean(process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY && process.env.RESEND_API_KEY);
}
...
export function requestFingerprint(request: Request) {
  ...
  return createHash("sha256").update(`${forwarded}|${agent}|${process.env.SUPABASE_SERVICE_ROLE_KEY ?? ""}`).digest("hex");
}

export function siteUrl() {
  return (process.env.NEWSLETTER_SITE_URL || "https://www.izzatbek-edu-group.uz").replace(/\/$/, "");
}
...
export async function sendNewsletterEmail({ to, subject, html }: { to: string; subject: string; html: string }) {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: process.env.NEWSLETTER_FROM || 'IZZATBEK-EDU-GROUP <news@izzatbek-edu-group.uz>',
      to: [to],
      subject,
      html,
    }),
