...
type ContentRow = {
  ...
  category: string | null; department: string | null; event_date: string | null; recipient_uz: string | null; recipient_en: string | null; source_url: string | null;
};
...
export async function loadPublishedContent(): Promise<PublishedContent> {
  ...
  return {
    ...
    news: rows.filter((row) => row.type === "news").map((row) => ({ slug: row.slug, status: "published", category: row.category === "announcement" ? "announcement" : "news", date: row.event_date ?? "", title: { uz: row.title_uz, en: row.title_en }, excerpt: { uz: row.summary_uz, en: row.summary_en }, body: { uz: splitBody(row.body_uz), en: splitBody(row.body_en) } })),
    achievements: rows.filter((row) => row.type === "achievement").map((row) => ({ slug: row.slug, status: "published", date: row.event_date ?? "", title: { uz: row.title_uz, en: row.title_en }, recipient: { uz: row.recipient_uz ?? "", en: row.recipient_en ?? "" }, summary: { uz: row.summary_uz, en: row.summary_en }, source: row.source_url ?? "" })),
  };
}
