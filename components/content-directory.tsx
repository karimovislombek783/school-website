...
export function AchievementDirectory({ lang, items }: { lang: Lang; items: AchievementRecord[] }) {
  const [category, setCategory] = useState<"all" | AchievementRecord["category"]>("all");
  ...
  const visible = items.filter((item) => {
    ...
    return (certificateType === "all" || item.credentialType === certificateType)
      && (result === "all" || item.result === result)
      && (year === "all" || item.academicYear === year);
  });
  const categories: Array<["all" | AchievementRecord["category"], string]> = [
    ["all", lang === "uz" ? "Barchasi" : "All achievements"],
...
function AchievementCard({ lang, item }: { lang: Lang; item: AchievementRecord }) {
  ...
  return <Dialog><article className={`achievement-card achievement-${item.category}`}>
    ...
    <div className="achievement-card-body"><span>{categoryLabel}</span><h2>{item.studentName}</h2>{item.subject[lang] && <p className={`achievement-subject-badge ${subjectTone}`}><AchievementSubjectIcon subject={item.subject[lang]} />{item.subject[lang]}</p>}<time>{item.academicYear}</time>
    ...
    <div className={`achievement-dialog-facts ${item.subject[lang] ? "has-subject" : ""}`}>
      ...
      {item.subject[lang] && <div className={`achievement-dialog-fact achievement-dialog-subject ${subjectTone}`}><AchievementSubjectIcon subject={item.subject[lang]} /><div><span>{lang === "uz" ? "Fan" : "Subject"}</span><strong>{item.subject[lang]}</strong></div></div>}
...
function AchievementSubjectIcon({ subject }: { subject: string }) {
  const value = subject.toLocaleLowerCase();
  if (/math|matemat/.test(value)) return <Sigma aria-hidden="true" />;
  if (/english|ingliz/.test(value)) return <BookOpen aria-hidden="true" />;
  if (/uzbek|o['‘’`]zbek|ona tili|adabiyot|literature/.test(value)) return <Languages aria-hidden="true" />;
  if (/biology|biolog/.test(value)) return <Dna aria-hidden="true" />;
  if (/chemistry|kimyo/.test(value)) return <FlaskConical aria-hidden="true" />;
  if (/history|tarix/.test(value)) return <Landmark aria-hidden="true" />;
